...
(ui/defcomponent save-button [document owner _]
...
(ui/defcomponent update-inbox-button [inbox owner _]
  (render [_]
...
(ui/defcomponent ^:private inbox-title [inbox owner _]
  (render [_]
   [:header.inbox
...
(ui/defcomponent ^:private editable-title [document owner _]
  (init-state [_]
    {:editing? false})
...
(ui/defcomponent ^:private document-page [page owner _]
  (init-state [_]
    {:selected? false
     ;; :drag-active? false
     ;; :drag-target nil
  ...
  (render-state [_ {:keys [selected? view ;; drag-active? drag-target
    ...
    (let [id (:id page)]
      [:li {:key id, :ref id
            :class ["page" (when selected? "selected")]
            :on-click (partial page-click page owner)}
...
(ui/defcomponent document [document owner _]
  (init-state [_]
...
(ui/defcomponent group-pages-workflow [state owner]
  (init-state [_]
    {:events (async/chan)})
  (will-mount [_]
